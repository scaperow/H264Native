...
//				"video/mp4v-es", 320, 240);
//		mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, 10200);
//		mediaFormat.setInteger(MediaFormat.KEY_FRAME_RATE, 15);
//		mediaFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT,
//				MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar);
//		mediaFormat.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, 5);
//
//		testEncoder("M4vH263Encoder", mediaFormat);
		


		// LinkedList<MediaFormat> formats = new LinkedList<MediaFormat>();
		//
		// final int kBitRates[] = { 4750, 5150, 5900, 6700, 7400, 7950, 10200,
		// 12200 };
		//
	...
	public void testAMRWBEncoders() {
		...
		final int kBitRates[] = { 6600, 8850, 12650, 14250, 15850, 18250,
				19850, 23050, 23850 };
	...
	public void testAACEncoders() {
		...
		final int kAACProfiles[] = { 2 /* OMX_AUDIO_AACObjectLC */,
				5 /* OMX_AUDIO_AACObjectHE */, 39 /* OMX_AUDIO_AACObjectELD */
		};
		...
		for (int k = 0; k < kAACProfiles.length; ++k) {
			for (int i = 0; i < kSampleRates.length; ++i) {
				if (kAACProfiles[k] == 5 && kSampleRates[i] < 22050) {
					// Is this right? HE does not support sample rates <
					// 22050Hz?
					continue;
				}
				for (int j = 0; j < kBitRates.length; ++j) {
	...
	private int queueInputBuffer(MediaCodec codec, ByteBuffer[] inputBuffers,
			int index) {
	...
	private void dequeueOutputBuffer(MediaCodec codec,
			ByteBuffer[] outputBuffers, int index, MediaCodec.BufferInfo info) {
	...
	private void testEncoder(String componentName, MediaFormat format) {
		...
		try {
			...
			// Log.e(TAG, "codec '" + componentName +
			// "' failed configuration.");
			e.printStackTrace();

			// assertTrue("codec '" + componentName + "' failed configuration.",
			// false);
		}
			...
			if (!doneSubmittingInput) {
				...
				if (index != MediaCodec.INFO_TRY_AGAIN_LATER) {
					if (numBytesSubmitted >= kNumInputBytes) {
						codec.queueInputBuffer(index, 0 /* offset */,
								0 /* size */, 0 /* timeUs */,
						...
						int size = queueInputBuffer(codec, codecInputBuffers,
								index);

						numBytesSubmitted += size;

						if (VERBOSE) {
							Log.d(TAG, "queued " + size
									+ " bytes of input data.");
						}
					}
				}
		...
		float actualRatio = (float) numBytesDequeued
				/ (float) numBytesSubmitted;

		if (actualRatio < 0.9 * desiredRatio
				|| actualRatio > 1.1 * desiredRatio) {
			Log.w(TAG, "desiredRatio = " + desiredRatio + ", actualRatio = "
					+ actualRatio);
		}
